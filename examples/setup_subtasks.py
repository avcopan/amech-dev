...
def setup_subtasks(
    ...
    """
    return tuple(setup_subtask_group(path, t, s) for t, s in groups)
...
def setup_subtask_group(
    ...
    def _subtask_path(root_path, task_dir, key):
        id_str_ = "{:02d}".format
        subtask_dir = key
        ...
        return root_path / task_dir / subtask_dir
    ...
    subtask_df = pandas.DataFrame(row_dcts)
    return subtask_df


def run_parallel(source_dir, nodes):
    import subprocess
...
if __name__ == "__main__":
    els_spc_df, els_pes_df, thermo_df, kin_df = setup_subtasks(
        path="propyl",
    )
    print(f"Elstruct species:\n{els_spc_df}")
    print(f"Elstruct transition states:\n{els_pes_df}")
    print(f"Thermo:\n{thermo_df}")
    print(f"Kinetics:\n{kin_df}")
    # run_parallel(
    #     nodes=["csed-0009", "csed-0010", "csed-0011", "csed-0012", "csed-0013"],
    # )
